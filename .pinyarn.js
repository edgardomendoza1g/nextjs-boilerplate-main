...
const config = {
  ...
  yarnUrl:
    'https://raw.githubusercontent.com/yarnpkg/berry/%40yarnpkg/cli/4.0.2/packages/yarnpkg-cli/bin/yarn.js',
};
      ...
      .get(
        ...
        (res) => {
          if (res.statusCode === 301 || res.statusCode === 302) {
            ...
            throw new Error(
              `Error downloading ${url}, status: ${res.statusCode}`
            );
          } else {
...
if (PLUGIN_LIST.length === 0) {
  ...
  for (const entry of entries) {
    const [, plugin, pluginHash] = entry.match(
      /plugin-(.*?)(?:-)?([0-9a-f]{8})?\.cjs/
    );
    const pluginUrl = (config.pluginUrls || {})[plugin];
    if (
